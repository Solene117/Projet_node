          ...
          <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6 w-full max-w-md">
            ...
            <form @submit.prevent="editingLocker ? updateLocker() : addLocker()" class="space-y-4">
              <div>
                ...
                <input 
                  v-model.number="newLocker.number" 
                  type="number"
                  placeholder="Numéro du casier" 
                  required 
              ...
              <div>
                ...
                <input 
                  v-model.number="newLocker.price" 
                  type="number"
                  step="0.01"
...
const addLocker = async () => {
  try {
    await axios.post('http://localhost:3033/api/lockers', newLocker.value);
...
const deleteLocker = async (id) => {
  try {
    await axios.delete(`http://localhost:3033/api/lockers/${id}`);
    fetchLockers();
  } catch (error) {
